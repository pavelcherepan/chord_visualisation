...
class ChordNameGenerator:
    ...
    def indentify_chord_quality(self, diagram: ChordDiagram) -> str | None:
        ...
        scale = self._create_chromatic_scale(self.root_note_name)
    ...
    def _create_chromatic_scale(self, root_note: str):
        return ChromaticScale(root_note)
    
    def _get_all_other_notes(self, diagram: ChordDiagram):
        # get names of all other notes in the chord
        return [
            self.fretboard.get_note(pos.string, pos.fret) for pos in diagram.shape
        ]
    
    def _calculate_distances(self, scale: ChromaticScale, notes: list[str]) -> list[int]:
        return [scale.scale[i] for i in notes]
    ...
    def _create_intervals(self, root_note: str, notes: list[str], distances: list[int]):
        ...
        for note, dist in zip(notes, distances):
            ...
            valid_intervals = [
                i[1] for i in Intervals._member_map_.items() if i[1].value == dist
            ]
